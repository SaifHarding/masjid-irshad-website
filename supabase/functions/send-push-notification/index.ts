...
// Notification types that map to preference columns
...
Deno.serve(async (req) => {
  ...
  try {
    const { title, body, tag, data, url, notification_type } = await req.json();

    console.log(`[send-push] Sending notification: ${title} (type: ${notification_type || "general"})`);
    ...
    // Filter subscriptions based on notification_type preferences
    const notifType: NotificationType = notification_type || "general";
    const preferencesFilteredSubs = subscriptions.filter((sub) => {
      // "general" type always sends to everyone
      if (notifType === "general") return true;
      
      // Check the corresponding preference column
      const prefColumn = `notify_${notifType}` as keyof typeof sub;
      const prefValue = sub[prefColumn];
      
      // If preference is explicitly false, skip this subscription
      if (prefValue === false) {
        console.log(`[send-push] Skipping ${sub.endpoint.slice(0, 40)}... (${notifType} disabled)`);
        return false;
      }
      return true;
    });

    const prefFilteredCount = subscriptions.length - preferencesFilteredSubs.length;
    console.log(`[send-push] After preference filter: ${preferencesFilteredSubs.length}/${subscriptions.length} (${prefFilteredCount} filtered out for ${notifType})`);

    if (preferencesFilteredSubs.length === 0) {
      console.log("[send-push] All subscriptions filtered by preferences");
      return new Response(
        JSON.stringify({ success: true, sent: 0, filtered: prefFilteredCount, message: "All subscriptions filtered by preferences" }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    const eligibleSubscriptions = preferencesFilteredSubs.filter((sub) => {
