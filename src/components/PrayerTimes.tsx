...
import { Clock, Loader2, Sun, AlertTriangle, CalendarDays, BookOpen } from "lucide-react";
    ...
    <section id="prayer-times" className="py-6 sm:py-8 md:py-16 bg-secondary/30">
      <div className="container px-3 sm:px-4">
        <div className="text-center mb-4 sm:mb-6 md:mb-12">
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <span>
                    <NotificationBell size="xs" />
                  </span>
                </TooltipTrigger>
                <TooltipContent>
                  <p>Get prayer time notifications</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
          <Link 
            to="/prayer-calendar" 
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs text-muted-foreground hover:text-foreground bg-muted/50 hover:bg-muted rounded-md transition-colors mb-4"
          >
              ...
              <div className="mb-3 md:mb-6">
                ...
                {hijriData && (
                  <p className="text-xs md:text-sm text-muted-foreground">
                    {hijriData.day} {hijriData.month.en} {hijriData.year} هـ
                  </p>
                )}
              </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {prayerData && specialTimes && (
            <div className="mt-4 md:mt-6 text-center">
              ...
              {/* Maktab promotion banner */}
              <Link 
                to="/maktab"
                className="group inline-flex flex-col items-center gap-0.5 mt-6 md:mt-8 px-5 py-2.5 text-sm bg-card hover:bg-muted/60 border border-border/50 hover:border-primary/30 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 animate-fade-in"
              >
                <span className="flex items-center gap-2 text-foreground font-medium">
                  <BookOpen className="h-4 w-4 text-primary" />
                  <span>Explore Weekday Maktab Classes</span>
                  <span className="text-primary transition-transform group-hover:translate-x-0.5 motion-safe:animate-[pulse_2s_ease-in-out_3]">→</span>
                </span>
                <span className="text-xs text-muted-foreground">
                  Join {Math.floor((studentCount?.total ?? 70) / 10) * 10}+ students
                </span>
              </Link>
            </div>
          )}
