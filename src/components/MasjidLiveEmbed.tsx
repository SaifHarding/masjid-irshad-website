...
import { Radio, PlayCircle, Bell, BellOff, BellRing, Share, MoreVertical, Download, Loader2, Clock } from "lucide-react";
...
const MasjidLiveEmbed = ({ station = "masjidirshad" }: MasjidLiveEmbedProps) => {
  ...
  const { isSupported, isSubscribed, isLoading: isNotificationLoading, permissionState, toggleSubscription } = useLiveNotifications(station);
  ...
  const handleToggleNotifications = useCallback(async () => {
    ...
    if (mobilePlatform && !isInstalledAsPWA() && !isSubscribed) {
    ...
    if (wasSubscribed) {
      ...
      toast.success("Notifications enabled! You'll be notified when Masjid Irshad goes live.");
    }
  }, [isSubscribed, isSupported, permissionState, toggleSubscription, mobilePlatform]);
      ...
      <div className="container space-y-3">
        ...
        <div className="flex items-center justify-between gap-4 px-4 py-3 rounded-lg bg-primary/10 border border-primary/20">
          ...
          <Button
            ...
            onClick={handleToggleNotifications}
            disabled={isNotificationLoading}
            className={`gap-2 shrink-0 ${isSubscribed ? "text-primary" : "text-muted-foreground"}`}
          >
            {isNotificationLoading ? (
              <>
                <Loader2 className="h-4 w-4 animate-spin" />
